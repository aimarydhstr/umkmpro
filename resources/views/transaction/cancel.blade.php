<div id="deleteModal"
    ...
    <div class="bg-white text-slate-700 dark:text-slate-200 dark:bg-slate-600 p-7 rounded-xl max-w-96">
        <p class="font-semibold pb-3 text-lg">Konfirmasi Batal Transaksi</p>
        <span>Apakah Anda yakin akan membatalkan transaksi ini?</span>
        <div class="flex mt-7 justify-end">
            <button id="batal"
                class="bg-white dark:bg-slate-600 dark:border-slate-600 dark:hover:border-slate-500 dark:hover:bg-slate-500 dark:active:border-slate-400 dark:active:bg-slate-400 text-sm w-20 text-center p-2 border border-white rounded-lg mr-3 hover:shadow-lg hover:border-slate-200 active:bg-slate-200 transition-all duration-300 focus:outline-none">Close</button>
            <form action="{{ route('shop.transactioncancel', $transaction->id) }}" method="get">
                @csrf
                <button type="submit"
                    class="bg-red-500 text-white text-sm border border-red-500 w-20 text-center shadow-lg p-2 rounded-lg hover:bg-red-600 active:border-red-500 active:bg-red-700 active:border-red-800 focus:outline-none">Batalkan</button>
            </form>
        </div>
    </div>
</div>